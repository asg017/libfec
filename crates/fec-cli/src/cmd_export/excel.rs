...
use fec_parser::{mappings::{column_names_for_field, DATE_COLUMNS, FLOAT_COLUMNS}, schedules::{form_type_schedule_type, ScheduleType}, FilingRow};
...
fn write_schedule_row(
  ...
  let state = sheets.entry(schedule.clone()).or_insert_with(|| {
    ...
    ScheduleSheetState { row_idx: 1, worksheet: new_ws, column_types }
  });
    ...
    match state.column_types.get(idx) {
      Some(FieldFormat::Float) => {
        if let Ok(num) = v.parse::<f64>() {
          state.worksheet.write_number_with_format(state.row_idx, idx as u16, num, &Format::new().set_num_format("$,0.00"))?;
        }
      },
      Some(FieldFormat::Date) => {
        match jiff::civil::Date::strptime("%Y%m%d", &v) {
          Ok(date) => {
            let d = ExcelDateTime::from_ymd(date.year().try_into().unwrap(), date.month().try_into().unwrap(), date.day().try_into().unwrap()).unwrap();
            state.worksheet.write_datetime_with_format(state.row_idx, idx as u16, d, &Format::new().set_num_format("yyyy-mm-dd"))?;
...
pub fn cmd_export_excel(
  ...
  assert!(filings.len() == 1, "Only one filing supported for Excel export");
  ...
  while let Some(Ok(row)) = filing.next_row() {
    match form_type_schedule_type(&row.row_type) {
      Some(schedule) => {
        write_schedule_row(&mut schedule_sheets, schedule, &filing.header.fec_version, &row)?;
      }
      None => {
    ...
    /*ws.write_string(row_idx, 0, row.row_type)?;
    for (idx, v) in row.record.iter().enumerate() {
      ws.write_string(row_idx, (idx + 1).try_into().unwrap(), v)?;
    }*/
  }

  /*for (name, ScheduleSheetState{worksheet: mut ws, ..}) in schedule_sheets {
    ws.autofit_to_max_width(300);
    workbook.push_worksheet(ws);
  }*/
  let mut schedule_sheets: Vec<(ScheduleType, ScheduleSheetState)> = schedule_sheets.into_iter().collect();
  schedule_sheets.sort_by_key(|(_, ws)| ws.worksheet.name());
  
  for (_, ScheduleSheetState{worksheet: mut ws, ..}) in schedule_sheets {
    ws.autofit_to_max_width(300);
    workbook.push_worksheet(ws);
  }
