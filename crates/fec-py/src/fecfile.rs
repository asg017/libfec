...
pub fn loads<'py>(py: Python<'py>, input: &Bound<'_, PyAny>, options: Option<&Bound<'py, PyDict>>) -> PyResult<Bound<'py, PyDict>> {
    // Extract options
...
#[pyfunction]
#[pyo3(signature = (hdr))]
pub fn parse_header<'py>(py: Python<'py>, hdr: &Bound<'_, PyAny>) -> PyResult<(Bound<'py, PyDict>, String, usize)> {
...
pub fn parse_line<'py>(
    ...
    let form_type = record.get(0)
        .ok_or_else(|| pyo3::exceptions::PyValueError::new_err("Line has no fields"))?;
    
    // Get column names for this form type
    let column_names: Vec<String> = match fec_parser::mappings::column_names_for_field(form_type, &version) {
        Ok(names) => names.to_vec(),
        Err(_) => {
            // If no mapping found, use generic field names
            (0..record.len())
...
pub fn from_http<'py>(
    ...
    match urlopen.call1((primary_url,)) {
        ...
        Err(_) => {
            // Try fallback URL
            let fallback_url = format!("https://docquery.fec.gov/paper/posted/{}.fec", file_num_str);
...
pub fn print_example(parsed: &Bound<'_, PyDict>) -> PyResult<()> {
    let itemizations_item = parsed.get_item("itemizations")?
